...
//have a type unto themselves and variables of a given function type can be declared and passed to another function.
...
fn straight_line_function(m: i32, c: i32, xses: &[i32]) -> Vec<(i32, i32)> { //the this argument is an array slice so as not to limit the number
                                                                          // of elements we want to pass. Other wise we have to use something like [i32,x]  where x is a specific number
...
pub fn run() {
    let ff = add; //Assigning a variable ff the function add
    let x = 7;
    let y = 8;
