...
impl Into<Circle> for Rect {
    fn into(self) -> Circle {
        let radius = f32::sqrt((self.area() as f32/3.141592653));//first convert the area of the rectangle (for rect so self is rect) the divide by pi anf find the square root
        radius
    }
}
